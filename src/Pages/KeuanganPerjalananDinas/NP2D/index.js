...
import { DeleteDataNp2d, GetAllDataNp2d, GetDataNp2dById } from "Services";
...
export const Np2d = () => {
    ...
    const fetchDataById = async id => {
        try {
            const response = await GetDataNp2dById(id);
            if (response.data) {
    ...
    const fetchAllData = async (value) => {
        try {
            const response = await GetAllDataNp2d({page: value, perpage: 10});
    ...
    const deleteData = async () => {
        try {
            const response = await DeleteDataNp2d(state.selectedId);
            if (response.data) {
                fetchAllData(1);
