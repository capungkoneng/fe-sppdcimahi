...
import { BerkendaraSchema } from "./data/BerkendaraSchema";
import { useRef, useEffect, useState } from "react";
import { AddDataBerkendara, EditDataBerkendara } from "Services/Pengaturan";
...
export const FormInput = ({
    ...
    const addData = async (payload) => {
        try {
            const response = await AddDataBerkendara(payload);
            if (response.data) {
    ...
    const editData = async (payload) => {
        ...
        try {
            const response = await EditDataBerkendara(item?.id,payload);
            if (response.data) {
                onCallback({success: true});
    ...
    return (
        <Formik
            ...
            initialValues={data}
            enableReinitialize
            validationSchema={BerkendaraSchema}
                    ...
                    >
                        ...
                        <div className="mt-8 flex justify-end">
                            <div className="flex gap-2 items-center">
                                ...
                                >
                                    {contentType === 'Add' ? 'Tambah Kendaraan' : 'Edit Kendaraan' }
                                </button>
                            </div>
                        </div>
                </Form>
