...
import { DataLabelPangkat } from "./data/tabelPangkat";
import { setContentType, setSelectedId } from "Configs/Redux/reducers";
import { useEffect, useState } from "react"
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";
import { GetAllPagePangkat, DeleteDataPangkat } from "Services/Pengaturan";
...
export const Pangkat = () => {
    ...
    const fetchAllData = async (value) => {
        try {
            const response = await GetAllPagePangkat({page: value, perpage: 10});
    ...
    const deleteData = async () => {
        try {
            const response = await DeleteDataPangkat(state.selectedId);
            if (response.data) {
                fetchAllData(1);
    ...
    return (
        <main>
            ...
            <Content 
                ...
                listContentTab={DataLabelPangkat}
