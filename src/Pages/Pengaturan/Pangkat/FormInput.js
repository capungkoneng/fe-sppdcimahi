...
import { PangkatSchema } from "./data/PangkatSchema";
import { useRef, useEffect, useState } from "react";
import { AddDataPangkat, EditDataPangkat } from "Services/Pengaturan";
...
export const FormInput = ({
    ...
    const addData = async (payload) => {
        try {
            const response = await AddDataPangkat(payload);
            if (response.data) {
    ...
    const editData = async (payload) => {
        ...
        try {
            const response = await EditDataPangkat(item?.id,payload);
            if (response.data) {
                onCallback({success: true});
    ...
    return (
        <Formik
            ...
            initialValues={data}
            enableReinitialize
            validationSchema={PangkatSchema}
                    ...
                    >
                        ...
                        <div className="mt-8 flex justify-end">
                            <div className="flex gap-2 items-center">
                                ...
                                >
                                    {contentType === 'Add' ? 'Tambah Jabatan' : 'Edit Jabatan' }
                                </button>
                            </div>
                        </div>
                </Form>
