...
import { BidangSchema } from "./data/BidangSchema";
import { useRef, useEffect, useState } from "react";
import { AddDataBidang, EditDataBidang } from "Services/Pengaturan";
...
export const FormInput = ({
    ...
    const addData = async (payload) => {
        try {
            const response = await AddDataBidang(payload);
            if (response.data) {
    ...
    const editData = async (payload) => {
        ...
        try {
            const response = await EditDataBidang(item?.id,payload);
            if (response.data) {
                onCallback({success: true});
                    ...
                    >
                        ...
                        <div className="mt-8 flex justify-end">
                            <div className="flex gap-2 items-center">
                                ...
                                >
                                    {contentType === 'Add' ? 'Tambah Bidang' : 'Edit Bidang' }
                                </button>
                            </div>
                        </div>
                </Form>
