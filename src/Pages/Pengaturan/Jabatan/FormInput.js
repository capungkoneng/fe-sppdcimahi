...
import { JabatanSchema } from "./data/JabatanSchema";
import { useRef, useEffect, useState } from "react";
import { AddDataJabatan, EditDataJabatan } from "Services/Pengaturan";
...
export const FormInput = ({
    ...
    const addData = async (payload) => {
        try {
            const response = await AddDataJabatan(payload);
            if (response.data) {
    ...
    const editData = async (payload) => {
        ...
        try {
            const response = await EditDataJabatan(item?.id,payload);
            if (response.data) {
                onCallback({success: true});
