...
import { DataLabelSubKegiatan } from "./data/tabelSubKegiatan";
import { setContentType, setSelectedId } from "Configs/Redux/reducers";
import { useEffect, useState } from "react"
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";
import { GetAllPageSubKegiatan, DeleteDataSubKegiatan, GetAllKegAnggaran } from "Services/Pengaturan";
import { FormInput } from "./FormInput";
// import { View } from "./View"

export const SubKegiatan = () => {
    const state = useSelector(state => state.root);
    const dispatch = useDispatch();
    const [listData, setListData] = useState([]);
    const [listDataKegAnggaran, setListDataKegAnggaran] = useState([]);
    ...
    const fetchAllData = async (value) => {
        try {
            const response = await GetAllPageSubKegiatan({page: value, perpage: 10});
    ...
    const fetchAllDataKegAnggaran = async () => {
        try {
            const response = await GetAllKegAnggaran();
            if (response.data) {
                setListDataKegAnggaran(response.data.msg);
            }
        } catch (error) {
            console.log(error);
            setListDataKegAnggaran([]);
        }
    }

    const deleteData = async () => {
        try {
            const response = await DeleteDataSubKegiatan(state.selectedId);
            if (response.data) {
                fetchAllData(1);
    ...
    const handleActionContent = (type, value) => {
        ...
        dispatch(setSelectedId(value.subkeg_id));
    }
    ...
    return (
        <main>
            ...
            <Content 
                ...
                listContentTab={DataLabelSubKegiatan}
                ...
                renderContent={(value) => {
                    return value.length === 0 ? (
                        ...
                        return (
                            <tr key={index}>
                                <TableContent className={`${index === value.length - 1 ? 'rounded-bl-lg' : '' }`}>{ result.kode_kegiatan_anggaran }</TableContent>
                                <TableContent>{ result.kode_sub_anggaran }</TableContent>
                                <TableContent className="w-[50%]">{ result.nama_sub_anggaran }</TableContent>
                ...
                {/* {
                    state.contentType === 'View' ? (
                        ...
                        <FormInput 
                            ...
                            item={data}
                            ListKegAnggaran={listDataKegAnggaran}
                            onCallback={(value) => {
