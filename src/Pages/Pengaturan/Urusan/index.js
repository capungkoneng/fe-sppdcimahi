...
import { DataLabelUrusan } from "./data/tabelUrusan";
import { setContentType, setSelectedId } from "Configs/Redux/reducers";
import { useEffect, useState } from "react"
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";
import { GetAllPageUrusan, DeleteDataUrusan } from "Services/Pengaturan";
...
export const Urusan = () => {
    ...
    const fetchAllData = async (value) => {
        try {
            const response = await GetAllPageUrusan({page: value, perpage: 10});
    ...
    const deleteData = async () => {
        try {
            const response = await DeleteDataUrusan(state.selectedId);
            if (response.data) {
                fetchAllData(1);
    ...
    const handleActionContent = (type, value) => {
        ...
        dispatch(setSelectedId(value.urusan_id));
    }
    ...
    return (
        <main>
            ...
            <Content 
                ...
                listContentTab={DataLabelUrusan}
                    ...
                    return value.length === 0 ? (
                        ...
                        return (
                            <tr key={index}>
                                ...
                                <TableContent>{ result.nama_urusan }</TableContent>
