...
import { DataLabelKegiatanAnggaran } from "./data/tabelKegiatanAnggaran";
import { setContentType, setSelectedId } from "Configs/Redux/reducers";
import { useEffect, useState } from "react"
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";
import { GetAllPageKegAnggaran, DeleteDataKegAnggaran, GetAllProgram } from "Services/Pengaturan";
import { FormInput } from "./FormInput";
// import { View } from "./View"

export const KegiatanAnggaran = () => {
    const state = useSelector(state => state.root);
    const dispatch = useDispatch();
    const [listData, setListData] = useState([]);
    const [listDataProgram, setListDataProgram] = useState([]);
    ...
    const fetchAllData = async (value) => {
        try {
            const response = await GetAllPageKegAnggaran({page: value, perpage: 10});
            if (response.data) {
    ...
    const fetchAllDataProgram = async () => {
        try {
            const response = await GetAllProgram();
            if (response.data) {
                setListDataProgram(response.data.msg);
            }
        } catch (error) {
            console.log(error);
            setListDataProgram([]);
    ...
    const deleteData = async () => {
        try {
            const response = await DeleteDataKegAnggaran(state.selectedId);
            if (response.data) {
                fetchAllData(1);
    ...
    const handleActionContent = (type, value) => {
        ...
        dispatch(setSelectedId(value.keg_id));
    }
    ...
    return (
        <main>
            ...
            />
            <Content 
                content="Kegiatan Anggaran"
                data={listData}
                // listTabData={DataTabsSupplier}
                listContentTab={DataLabelKegiatanAnggaran}
                ...
                renderContent={(value) => {
                    return value.length === 0 ? (
                        ...
                        return (
                            <tr key={index}>
                                <TableContent className={`${index === value.length - 1 ? 'rounded-bl-lg' : '' }`}>{ result.kode_program }</TableContent>
                                <TableContent>{ result.kode_kegiatan_anggaran }</TableContent>
                                <TableContent className="w-[50%]">{ result.nama_kegiatan_anggaran }</TableContent>
