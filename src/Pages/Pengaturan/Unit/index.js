...
import { DataLabelUnit } from "./data/tabelUnit";
import { setContentType, setSelectedId } from "Configs/Redux/reducers";
import { useEffect, useState } from "react"
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";
import { GetAllPageUnit, DeleteDataUnit, GetAllUrusan } from "Services/Pengaturan";
...
export const Unit = () => {
    ...
    const [listDataKodeUrusan, setListDataKodeUrusan] = useState([]);
    ...
    const fetchAllData = async (value) => {
        try {
            const response = await GetAllPageUnit({page: value, perpage: 10});
    ...
    const fetchAllDataUrusan = async () => {
        try {
            ...
            if (response.data) {
                setListDataKodeUrusan(response.data.msg);
            }
        } catch (error) {
            console.log(error);
            setListDataKodeUrusan([]);
        }
    }

    const deleteData = async () => {
        try {
            const response = await DeleteDataUnit(state.selectedId);
            if (response.data) {
                fetchAllData(1);
    ...
    const handleActionContent = (type, value) => {
        ...
        dispatch(setSelectedId(value.unit_id));
    }
                    ...
                    return value.length === 0 ? (
                        ...
                        return (
                            <tr key={index}>
                                ...
                                <TableContent>{ result.kode_unit }</TableContent>
                                <TableContent>{ result.nama_unit }</TableContent>
                ...
                {/* {
                    state.contentType === 'View' ? (
                        ...
                        <FormInput 
                            ...
                            item={data}
                            ListKodeUrusan={listDataKodeUrusan}
                            onCallback={(value) => {
